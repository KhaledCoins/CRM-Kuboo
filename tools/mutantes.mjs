...
const SUITES = {
  ...
  "src/lib/leads.ts": ["src/lib/__tests__/bolsao.test.mjs", "src/lib/__tests__/dinheiro.test.mjs", "src/lib/__tests__/telas.test.mjs"],
  "src/lib/tarefas.ts": ["src/lib/__tests__/telas.test.mjs"],
  "src/lib/c2s.ts": ["src/lib/__tests__/telas.test.mjs"],
  "src/pages/Bolsao.tsx": ["src/lib/__tests__/telas.test.mjs"],
  "src/components/Layout.tsx": ["src/lib/__tests__/telas.test.mjs"],
...
const MUTACOES = [
  ...
  ["src/lib/leads.ts", "contarPerdidos passa a contar venda GANHA como perda",
    '.not("etapa", "eq", "ganho")', ''],
  ["src/lib/tarefas.ts", "arrastar tarefa volta a mentir (update sem conferir linha)",
    'if (!data?.length) throw new Error(NADA_GRAVADO);', ''],
  ["src/lib/c2s.ts", "remover etiqueta volta a sumir da tela e ficar no banco",
    'if (!data?.length) { console.error("[c2s] removerEtiqueta: 0 linhas afetadas (RLS bloqueou ou já não existia)"); return false; }', ''],
  ["src/lib/leads.ts", "falha de rede volta a virar lista VAZIA (consultor acha que nao tem trabalho)",
    'return { leads: [], erro: error.message };', 'return { leads: [], erro: null };'],
  ["src/lib/leads.ts", "badge do bolsao volta a estampar 0 que nao apurou",
    'if (error) { console.error("[leads] contarBolsao:", error.message); return null; }', 'if (error) { console.error("[leads] contarBolsao:", error.message); }'],
  ["src/lib/leads.ts", "uma falha aborta a distribuicao inteira (leads seguintes nem sao tentados)",
    '      falhados++;', '      throw e;'],
  ["src/lib/leads.ts", "rodizio manual volta a comecar sempre no mesmo consultor",
    'let i = comecarEm;', 'let i = 0;'],
  ["src/pages/Bolsao.tsx", "botao Distribuir volta a travar quando algo falha",
    '      setDistribuindo(false);', '      // MUTADO: fora do finally'],
