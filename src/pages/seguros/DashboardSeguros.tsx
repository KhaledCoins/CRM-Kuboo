...
export function DashboardSeguros() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase.from("vendas").select("valor,comissao_valor,data_venda,produto,seguradora,vendedor_nome").gte("data_venda", monthPrefix() + "-01").limit(3000);
      if (!active) return;
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card pad={false}>
          ...
          {temDados ? <BarList items={m.topSeg} color="#2DD4A7" /> : <EmptyState icon={PieIcon} title="Nenhuma venda registrada este mês" />}
        </Card>
        <Card pad={false}>
