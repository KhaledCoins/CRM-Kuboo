...
function ToggleRow({ label, checked, onChange, hint, disabled }: { label: string; checked: boolean; onChange: (v: boolean) => void; hint?: string; disabled?: boolean }) {
  return (
    <label className={`flex items-center justify-between gap-3 ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}>
      ...
      <input type="checkbox" checked={checked} disabled={disabled} onChange={(e) => onChange(e.target.checked)} className="w-4 h-4 accent-brand-500 shrink-0" />
    </label>
  );
}
...
function AvisoSomenteLeitura() {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-muted flex items-start gap-2">
      <Lock size={16} className="mt-0.5 shrink-0" />
      <span>Você não tem permissão para editar esta seção — modo somente leitura.</span>
    </div>
  );
}
...
function AbaBolsao() {
  const { user } = useAuth();
  const podeEditar = can(user, "editar_bolsao");
  ...
  return (
    <div className="space-y-5">
      {erro && <AvisoMigracao />}
      {!erro && !podeEditar && <AvisoSomenteLeitura />}

      <Card>
        <ToggleRow label="Bolsão ativo" hint="Leads sem interação a tempo ficam disponíveis para qualquer um pegar" checked={cfg?.ativo ?? true} onChange={(v) => salvar({ ativo: v })} disabled={!podeEditar} />
      </Card>

      <Card>
        <span className={labelCls}>Limite de tempo para atendimento</span>
        <select className={`${inputCls} max-w-xs disabled:opacity-50`} disabled={!podeEditar} value={cfg?.limite_minutos ?? 20} onChange={(e) => salvar({ limite_minutos: Number(e.target.value) })}>
      ...
      <Card>
        ...
        <div className="space-y-2 mt-1">
          <label className={`flex items-center gap-2 text-sm text-ink ${podeEditar ? "cursor-pointer" : "opacity-50 cursor-not-allowed"}`}>
            <input type="radio" name="escopo" disabled={!podeEditar} className="accent-brand-500" checked={(cfg?.escopo ?? "empresa") === "empresa"} onChange={() => salvar({ escopo: "empresa" })} />
            Qualquer usuário da empresa pode pegar
          </label>
          <label className={`flex items-center gap-2 text-sm text-ink ${podeEditar ? "cursor-pointer" : "opacity-50 cursor-not-allowed"}`}>
            <input type="radio" name="escopo" disabled={!podeEditar} className="accent-brand-500" checked={cfg?.escopo === "equipe"} onChange={() => salvar({ escopo: "equipe" })} />
            ...
            return (
              <div key={d.chave} className="flex items-center gap-2 flex-wrap sm:flex-nowrap bg-slate-50 rounded-xl px-3 py-2">
                <label className="inline-flex items-center gap-2 w-24 shrink-0">
                  <input type="checkbox" disabled={!podeEditar} className="accent-brand-500" checked={c.ativo} onChange={(e) => setDiaHorario(d.chave, { ativo: e.target.checked })} />
                  <span className="text-xs font-bold text-ink">{d.rotulo}</span>
                </label>
                <input type="time" disabled={!c.ativo || !podeEditar} className={`${inputCls} disabled:opacity-40`} value={c.ini} onChange={(e) => setDiaHorario(d.chave, { ini: e.target.value })} />
                <span className="text-xs text-muted">até</span>
                <input type="time" disabled={!c.ativo || !podeEditar} className={`${inputCls} disabled:opacity-40`} value={c.fim} onChange={(e) => setDiaHorario(d.chave, { fim: e.target.value })} />
              </div>
            );
          })}
        </div>
...
function AbaAlertas() {
  const { user } = useAuth();
  const podeEditar = can(user, "acessar_config");
  ...
  return (
    <div className="space-y-4">
      ...
      {itens.length === 0 ? (
        ...
        <Card pad={false}>
          <div className="p-2">
            <Table head={<><Th>Sem atendimento há</Th><Th>Notificar</Th><Th>Status</Th>{podeEditar && <Th right>Ações</Th>}</>}>
              {itens.map((a) => (
                <Tr key={a.id}>
                  <Td>
                    <div className="flex items-center gap-2">
                      <input type="number" min={1} disabled={!podeEditar} className={`${inputCls} w-24 disabled:opacity-50`} value={a.minutos} onChange={(e) => salvar(a.id, { minutos: Number(e.target.value) || 1 })} />
                      <span className="text-xs text-muted">{minutosLabel(a.minutos)}</span>
                    </div>
                  </Td>
                  <Td>
                    <select className={`${inputCls} disabled:opacity-50`} disabled={!podeEditar} value={a.notificar} onChange={(e) => salvar(a.id, { notificar: e.target.value as AlertaConfig["notificar"] })}>
                      {NOTIFICAR_OPCOES.map((o) => <option key={o.valor} value={o.valor}>{o.rotulo}</option>)}
                    </select>
                  </Td>
                  <Td>
                    <label className={`inline-flex items-center gap-2 ${podeEditar ? "cursor-pointer" : "cursor-not-allowed"}`}>
                      <input type="checkbox" disabled={!podeEditar} className="accent-brand-500" checked={a.ativo} onChange={(e) => salvar(a.id, { ativo: e.target.checked })} />
...
function AbaMotivos() {
  ...
  async function criar() {
    if (!podeEditar || !novoNome.trim()) return;
  ...
  function iniciarEdicao(m: MotivoArquivamento) { if (!podeEditar) return; setEditandoId(m.id); setEditandoNome(m.nome); }
          ...
          {itens.map((m) => (
            <Card key={m.id} pad={false} className="flex items-center justify-between gap-2 p-3">
              {editandoId === m.id ? (
                ...
                <span className={`text-sm font-bold text-left truncate flex-1 ${m.ativo ? "text-ink" : "text-muted line-through"}`}>{m.nome}</span>
              )}
              <label className={`inline-flex items-center gap-1.5 shrink-0 ${podeEditar ? "cursor-pointer" : "cursor-not-allowed"}`} title={m.ativo ? "Desativar" : "Ativar"}>
                <input type="checkbox" disabled={!podeEditar} className="accent-brand-500" checked={m.ativo} onChange={() => alternar(m)} />
              </label>
            </Card>
          ))}
        </div>
...
function AbaEtiquetas() {
  const { user } = useAuth();
  const podeEditar = can(user, "editar_etiquetas");
          ...
          {itens.map((e) => (
            <Card key={e.id} pad={false} className={`flex items-center justify-between gap-2 p-3 ${!e.ativa ? "opacity-60" : ""}`}>
              <div className="flex items-center gap-2 min-w-0">
                <input type="color" value={e.cor} disabled={!podeEditar} onChange={(ev) => mudarCor(e, ev.target.value)} className="w-6 h-6 rounded-full border border-slate-200 cursor-pointer shrink-0 disabled:cursor-not-allowed" title="Mudar cor" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        {podeEditar && <Button icon={Plus} size="sm" onClick={() => setEditando("novo")}>Nova mensagem</Button>}
      </div>
