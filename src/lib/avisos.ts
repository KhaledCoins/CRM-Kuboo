...
export async function fetchAvisos(modulo: "seguros" | "consorcios"): Promise<Aviso[]> {
  ...
  const [leadsR, vendasR, apolR] = await Promise.all([
    supabase.from("leads").select("id,nome,vendedor_id,sla_expira_em,primeiro_contato_em,etapa").not("vendedor_id", "is", null).is("primeiro_contato_em", null).not("sla_expira_em", "is", null).limit(200),
    supabase.from("vendas").select("id,cliente_nome,vigencia_fim").not("vigencia_fim", "is", null).neq("status", "cancelada").gte("vigencia_fim", hoje).lte("vigencia_fim", em7d).limit(50),
    supabase.from("apolices").select("id,tipo,vigencia_fim,profiles(name)").not("vigencia_fim", "is", null).neq("status", "cancelada").gte("vigencia_fim", hoje).lte("vigencia_fim", em7d).limit(50),
  ]);
  ...
  // Renovações da semana (vendas do CRM + carteira de apólices)
